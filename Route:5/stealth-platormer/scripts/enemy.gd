...
@onready var proximity_range: Sprite2D = $proximity_range
...
var walk_detectrange : float = 230
var breath_detectrange : float = 115
...
func _process(delta: float) -> void:
	if not player.radar_on:
		proximity_range.hide()
		return
	proximity_range.show()
...
func _physics_process(delta: float) -> void:
	...
	if not ray_cast.is_colliding():
		direction = 1
	elif not ray_cast_2d_2.is_colliding():
		direction = -1
	if direction == 1:
		animated_sprite_2d.flip_h = true
	else:
...
func _patrol():
	...
	normalsound.pitch_scale = lerp(normalsound.pitch_scale,1.0,0.1)
...
func _alert():
	if not normalsound.playing:
		normalsound.playing = true
	normalsound.pitch_scale = lerp(normalsound.pitch_scale,1.2,0.1)
...
func _stunned():
	if not normalsound.playing:
		normalsound.playing = true
	normalsound.pitch_scale = lerp(normalsound.pitch_scale,1.4,0.1)
